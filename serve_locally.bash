...
if [ $envsetup == "yes" ]; then
  ...
  echo "1.0  Install jekyll by following https://jekyllrb.com/docs/installation/ubuntu/"
  echo "1.1  First install ruby dependencies"
  echo ""
  echo "         sudo apt-get install ruby-full build-essential zlib1g-dev"
  echo ""
  echo "1.2  Set up gem installation directory for user (not root)"
  echo ""
  ...
  echo "         source ~/.bashrc"
  echo ""
  echo "2.0  Now install jekyll for serving and creating pages "
  echo "     and bundler for book keeping version numbers and dependencies"
  echo ""
  echo "         gem install jekyll bundler"
  echo ""
  echo "3.0 Initialize environment"
  echo ""
  echo "         bundle init"
  echo "         bundle add jekyll"
  echo ""
  echo "4.0 If you are using a theme from Github  in your _config.yml then it should "
  ...
  echo "         bundle install"
  echo ""
  echo "5.0 You should now be good to go!"
  exit
fi
